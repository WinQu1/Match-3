...
public class Board : MonoBehaviour
{
    ...
    public GameObject honeyPiecePrefab;
    ...
    private BackgroundTile[,] honeyTiles;
    ...
    void Start()
    {
        ...
        honeyTiles = new BackgroundTile[width, height];
    ...
    private void GenerateHoneyTiles()
    {
        //Look at all the tiles in the layout
        for (int i = 0; i < boardLayout.Length; i++)
        {
            //if a tile is a "Honey" tile
            if (boardLayout[i].tileKind == TileKind.Honey)
            {
                //Create a "Honey" tile at that position;
                Vector2 tempPosition = new Vector2(boardLayout[i].x, boardLayout[i].y);
                GameObject tile = Instantiate(honeyPiecePrefab, tempPosition, Quaternion.identity);
                honeyTiles[boardLayout[i].x, boardLayout[i].y] = tile.GetComponent<BackgroundTile>();
            }
        }
    }
    ...
    private void SetUp()
    {
        ...
        for (int i = 0; i < width; i++)
        {
            ...
            {
                if (!blankSpaces[i, j] && !concreteTiles[i, j] && !honeyTiles[i, j])
                {
    ...
    private void DamageHoney(int column, int row)
    {
        if (column > 0)
        {
            if (honeyTiles[column - 1, row])
            {
                honeyTiles[column - 1, row].TakeDamage(1);
                if (honeyTiles[column - 1, row].hitPoints <= 0)
                {
                    honeyTiles[column - 1, row] = null;
                }
                makehoney = false;
            }
        }
        ...
        if (row > 0)
        {
            if (honeyTiles[column, row - 1])
            {
                honeyTiles[column, row - 1].TakeDamage(1);
                if (honeyTiles[column, row - 1].hitPoints <= 0)
                {
                    honeyTiles[column, row - 1] = null;
                }
                makehoney = false;
    ...
    private IEnumerator DecreaseRowCo2()
    {
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                if (!blankSpaces[i, j] && allDots[i, j] == null && !concreteTiles[i, j] && !honeyTiles[i, j])
                {
    ...
    private void RefillBoard()
    {
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (allDots[i, j] == null && !blankSpaces[i, j] && !concreteTiles[i, j] && !honeyTiles[i, j])
                {
                    Vector2 tempPosition = new Vector2(i, j + offSet);
                    int dotToUse = Random.Range(0, dots.Length);
                    int maxIterations = 0;
    ...
    private void CheckToMakeHoney()
    {
        //Check the honey tiles array
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (honeyTiles[i, j] != null && makehoney)
                {
                    //Call another method to make a new honey
                    MakeNewHoney();
                    return;
                }
            }
    ...
    private void MakeNewHoney()
    {
        bool honey = false;
        int loops = 0;
        while (!honey && loops < 200)
        {
            int newX = Random.Range(0, width);
            int newY = Random.Range(0, height);
            if (honeyTiles[newX, newY] != null)
            {
                ...
                if (adjacent != Vector2.zero)
                {
                    ...
                    honeyTiles[newX + (int)adjacent.x, newY + (int)adjacent.y] = tile.GetComponent<BackgroundTile>();
                    honey = true;
                }
